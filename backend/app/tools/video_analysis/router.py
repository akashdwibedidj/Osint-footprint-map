...
from app.tools.video_analysis import service, utils

TOOL_ID = "video_analysis"
router = APIRouter(prefix="/video_analysis", tags=["video_analysis"])

VIDEO_UPLOAD_DIR = os.path.join(settings.upload_dir, "video")
...
async def upload_media(
    ...
    if not (utils.is_image(filename) or utils.is_video(filename)):
    ...
    original_path = os.path.join(VIDEO_UPLOAD_DIR, f"{scan.id}{ext}")

    with open(original_path, "wb") as f:
        f.write(await file.read())

    background_tasks.add_task(
        service.process_video_job,
        scan.id,
        label,
        original_path,
        filename,
...
def get_video_findings(value: str, db: Session = Depends(get_db)):
