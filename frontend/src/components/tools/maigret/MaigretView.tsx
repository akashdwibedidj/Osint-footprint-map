...
export default function MaigretView() {
  const [username, setUsername] = useState("");
  const [loading, setLoading] = useState(false);
  const [findings, setFindings] = useState<MaigretFinding[]>([]);
  ...
  const runScan = async () => {
    ...
    setError(null);
    try {
      await api.post(`/maigret/username/${username.trim()}`);
      const res = await api.get(`/maigret/username/${username.trim()}`);
          ...
          <table className="w-full text-sm font-mono">
            <thead>
              <tr className="bg-zinc-900 text-zinc-500 text-xs uppercase tracking-wider">
                <th className="text-left px-4 py-2 w-12">Img</th>
                ...
                return (
                  <tr key={idx} className="border-t border-zinc-800 text-zinc-300">
                    ...
                    <td className="px-4 py-2 text-zinc-400 text-xs">
                      {meta.fullname || "—"}
                      {meta.is_verified === "True" && <span className="ml-1 text-blue-400">✓</span>}
                    </td>
                    <td className="px-4 py-2">
